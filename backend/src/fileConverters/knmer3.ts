...
function watchForThirdPartyProcessing() {
    console.log(`[watchForThirdPartyProcessing] Watching: ${outputFolder}`);

    const thirdPartyOutputFolder = 'C:\\MESI services\\MESI mTABLET GDT Integration Service\\Output';
    const preservedListenerFile = 'mTABLET.IntegrationService.GDT.WaitForResult.exe';

    chokidar.watch(outputFolder, { persistent: true, depth: 0 })
        .on('unlink', (filePath) => {
            if (processedFiles.has(filePath)) return; // Avoid duplicate triggers
            processedFiles.add(filePath);
            console.log(`[watchForThirdPartyProcessing] Detected processed file: ${filePath}`);

            setTimeout(() => {
                console.log(`[watchForThirdPartyProcessing] Cleaning up...`);
        ...
        .on('error', (error) => console.error(`[watchForThirdPartyProcessing] Error: ${error}`));
}
